...
k6 run k6/smoke.js
# atau
BASE_URL=http://localhost:8000 EMAIL=you@example.com PASSWORD=secret k6 run k6/smoke.js
```

-   Load test (ramping stages):

```bash
k6 run k6/load.js
# atur environment bila perlu
BASE_URL=http://localhost:8000 EMAIL=you@example.com PASSWORD=secret k6 run k6/load.js
```

Tips:

-   Naikkan `RATE_LIMIT_MAX` sementara saat uji beban.
-   Jalankan DB/instance terpisah untuk tes tulis agar tidak mengganggu produksi.
