...
async function seedData() {
    try {
        ...
        if (seed.sample) {
            const adminUser = await User.findOne({ email: "iqbaldev.site@gmail.com" })
            if (adminUser) {
                ...
                if (existingRecords.length === 0) {
                    ...
                    console.log("✅ Sample tax records created (controlled by SEED_SAMPLE)")
                }
            }
        }

        // Clean orphaned records (always safe)
        const allUsers = await User.find()
        await TaxRecord.deleteMany({ user_id: { $nin: allUsers.map((u) => u._id) } })
